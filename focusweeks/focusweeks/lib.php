 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_focusweeks extends core_courseformat\base {
    ...
    public function uses_indentation(): bool {
        return (get_config('format_focusweeks', 'indentation')) ? true : false;
    }
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_focusweeks');
        } else {
    ...
    public function get_section_highlighted_name(): string {
        return get_string('currentsection', 'format_focusweeks');
    }
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $courseformatoptionsedit = array(
                ...
                'automaticenddate' => array(
                    'label' => new lang_string('automaticenddate', 'format_focusweeks'),
                    'help' => 'automaticenddate',
                    'help_component' => 'format_focusweeks',
                    'element_type' => 'advcheckbox',
                ), 
                'weekoffset' => array(
                    'label' => new lang_string('weekoffset', 'format_focusweeks'),
                    'help' => 'weekoffset',
                    'help_component' => 'format_focusweeks',
                    'element_type' => 'text',
                    'default' => 0,
            ...
            foreach ($courseformatoptionsedit as $key => $editopts) {
                if (isset($courseformatoptions[$key])) {
                    $courseformatoptions[$key] = array_merge($courseformatoptions[$key], $editopts);
                }
            }
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        $renderer = $PAGE->get_renderer('format_focusweeks');
    ...
    public static function update_end_date($courseid) {
        ...
        $course = $DB->get_record_sql($sql,
            ['optionname' => 'automaticenddate', 'format' => 'focusweeks', 'courseid' => $courseid]);
        ...
        $format = new format_focusweeks('focusweeks', $courseid);
    ...
    public function get_config_for_external() {
        ...
        $formatoptions['indentation'] = get_config('format_focusweeks', 'indentation');
 ...
 */
function format_focusweeks_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'focusweeks'), MUST_EXIST);
